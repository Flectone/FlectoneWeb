...
export default function Metric(props: MetricProps) {
  ...
  const processData = useCallback(
    (rawValues: MetricItem[]) => {
      ...
      if (convert === "gb") {
        ...
        processed = Array.from(groupedMap.entries()).map(([value, count]) => ({
          value,
          count,
        }))
      }
  ...
  useEffect(() => {
    ...
    const fetchGeoAndData = async () => {
      try {
        ...
        const res = await fetch(baseUrl + apiPath)
        const json = await res.json()
        const filteredValues = (json.values || []).filter(
          (item: MetricItem) =>
            item.value !== "null" && item.value !== "Unknown"
        )
    ...
    const fetchTwo = async () => {
      try {
        ...
        setFirstData(
          slice === true
            ? processData(firstJson.values || []).slice(1, -1)
            : processData(firstJson.values || [])
        )
        setSecondData(
          slice === true
            ? processData(secondJson.values || []).slice(1, -1)
            : processData(secondJson.values || [])
        )
      } catch (error) {
      ...
      series: [
        {
          ...
          itemStyle: {
            borderRadius: 0,
            borderColor: resolvedTheme === "dark" ? "#111" : "#fff",
            borderWidth: 0,
          },
          label: { show: false },
          emphasis: {
  ...
  const treemapOption = useMemo((): EChartsOption => {
    ...
    const formattedData = rawValues
      .map((item) => ({
        name: item.value,
        value: item.count,
        itemStyle: {
          color: item.count >= threshold ? "#21d17f" : "#ffcc00",
          borderRadius: 4,
        },
      }))
  ...
  const geoOption = useMemo((): EChartsOption => {
    ...
    return {
      ...
      visualMap: {
        ...
        textStyle: { color: resolvedTheme === "dark" ? "#999" : "#666" },
      },
      series: [
        {
          name: name,
          ...
          label: { show: false },
          itemStyle: {
            areaColor: resolvedTheme === "dark" ? "#1a1a1a" : "#f3f4f6",
            borderColor: resolvedTheme === "dark" ? "#333" : "#ccc",
          },
          emphasis: {
            label: { show: false },
            itemStyle: {
              areaColor: resolvedTheme === "dark" ? "#2db5aa" : "#2db5aa",
            },
          },
          nameProperty: "name",
          data: chartData.map((item) => ({
  ...
  const twoLineOption = useMemo((): EChartsOption => {
    ...
    return {
      ...
      legend: {
        data: [data?.first.name, data?.second.name],
        bottom: "93%",
      ...
      series: [
        {
          ...
          data: firstData.map((item) => item.count + " " + suffix),
          type: "line",
          smooth: true,
          color: "#21d17f",
        },
        {
          ...
          data: secondData.map((item) => item.count + " " + suffix),
          type: "line",
          smooth: true,
          color: "#377ded",
        },
  ...
  if (!mounted || loading || (type === "geo" && !mapLoaded)) {
    ...
    return (
      <div className={`${className}`}>
        <div className={`animate-pulse bg-fd-primary/10`}></div>
      </div>
    )
  }
