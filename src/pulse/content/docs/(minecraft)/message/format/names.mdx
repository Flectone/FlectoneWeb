...
Сообщения, которые будут форматироваться от отправителя на сервере, откуда было отправлено. Их можно использовать в любом сообщении с помощью `<constant:индекс>` (начиная с 0). Если указан некорретный индекс или его нет, будет использоваться первая константа. Константы форматируются каждый раз, когда обновляется `ticker`
...
Пересчитывает константы игрока. Работает только в режиме прокси, потому что без него константы считаются сразу при отправке сообщения
