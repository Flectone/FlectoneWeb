---
title: Чат
description: Модуль, отвечающий за отправленные сообщения игроком в чат
authors:
  - TheFaser
tag: "minecraft"
---

![chat](/pulse/message_chat.webp)

<Callout type="info" title="Информация">
Как отключить локальный чат? Внутри `message.yml` выключи `local` с помощью `enable: false`, а также сделай пустой `trigger: ""` для `global`

    ```yaml
    chat:
      enable: true
      mode: "BUKKIT"
      priority: "NORMAL"
      types:
        local:
          enable: false # поставь false
        global:
          enable: true
          cancel: false
          range: "PROXY"
          priority: 5
          trigger: "" # сделай пусто
          null_receiver:
            enable: true
            destination:
              type: "ACTION_BAR"
              times:
                stay: 20
          destination:
            type: "CHAT"
          cooldown:
            enable: false
          sound:
            enable: false
    ```
</Callout>

<Callout type="info" title="Информация">
    Как настроить чат, чтобы он работал с другими плагинами?

    Для TownyChat:
    1. Зайди в файл ChatConfig.yml (в папке Towny)
    2. Во вкладке modify_chat поставь `enable: false` (это означает, что он не будет модифицировать global type чаты)
    3. В самом конце файла allow_exclamation_point_to_shout поставь false (это нужно, чтобы не было проблем с "!" знаком)

    Внутри FlectonePulse:
    1. Зайди в message.yml
    2. В модуле `chat` поставь `mode: "BUKKIT"` (или `mode: "PAPER"` для некоторых плагинов) и `priority: "HIGHEST"` (чтобы FlectonePulse изменял чат только в самую последнюю очередь)

    Это решает проблемы с 99% плагинами, которые взаимодействуют с чатом. В крайнем случае можно поставить `mode: "PACKET"`, чтобы FlectonePulse обрабатывал сообщения чата через PacketEvents

</Callout>

<Callout type="info" title="Информация">
    Как сделать свой чат? Например админ-чат, который видно на всех серверах и который читают только админы

    <Steps>
        <Step>
            В `message.yml` добавь новый чат в `chat.types`, здесь `!!` это то, с чего должно начинаться сообщение
            ```yaml
            types:
              admin:
                enable: true
                cancel: true
                range: "PROXY"
                priority: 10
                trigger: "!!"
                null_receiver:
                  enable: false
                destination:
                  type: "CHAT"
                cooldown:
                  enable: false
                sound:
                  enable: false
            ```
        </Step>
        <Step>
            В `localizations → язык.yml` добавь формат этого чата в `message.chat.types`
            ```yaml
            types:
              admin: "<delete><color:#ff7171>[A] <display_name><fcolor:3>: <message>"
            ```
        </Step>
        <Step>
            В `permission.yml` добавь право в `message.chat.types`, иначе чат будет доступен всем
            ```yaml
            types:
              admin:
                name: "flectonepulse.module.message.chat.admin"
                type: "OP"
            ```
        </Step>
        <Step>
            Перезагрузи FlectonePulse с помощью `/flectonepulse reload` и напиши `!!привет`
        </Step>
    </Steps>

    Название чата используется и в других местах, например для отправки в Discord это будет `MESSAGE_CHAT_ADMIN`
</Callout>

<br></br>

# <include>../../../../parts/words.mdx#localization</include>
<span><include>../../../../parts/words.mdx#path</include> `localizations → язык.yml → message.chat`</span>

<Tabs items={['ru_ru', 'en_us']}>
    <Tab value="ru_ru"><include>../../../../files/minecraft/localizations/ru_ru.yml#chat</include></Tab>
    <Tab value="en_us"><include>../../../../files/minecraft/localizations/en_us.yml#chat</include></Tab>
</Tabs>

### `null_chat`

Сообщение, если в чат ничего не удалось отправить

### `null_receiver`

Сообщение, если никто не увидел сообщение игрока

### `types`

Список чатов и их формат

<br></br>

# <include>../../../../parts/words.mdx#setting</include>
<span><include>../../../../parts/words.mdx#path</include> `message.yml → chat`</span>

<include>../../../../files/minecraft/message.yml#chat</include>

<include>../../../../parts/enable.mdx</include>

### `mode`

Режим обработки чата

| Тип       | Объяснение                                                                               |
|-----------|------------------------------------------------------------------------------------------|
| `PACKET`  | Сообщения чата обрабатываются через пакеты (полностью асинхронно)                        |
| `BUKKIT`  | Сообщения чата обрабатываются через `Bukkit` (асинхронно, но только в одном потоке чата) |
| `PAPER`   | Сообщения чата обрабатываются через `Paper` (асинхронно, но только в одном потоке чата)  |

<Callout type="warn" title="Предупреждение">
Если тебе нужно, чтобы твой другой плагин видел сообщения чата, поставь `mode: "BUKKIT"`. В редком случае, если плагин работает через `Paper` слушатели, то нужно поставить `mode: "PAPER"`

</Callout>

### `priority`

Приоритет ивента сообщения в чате. Измените это значение, если есть конфликт с другими плагинами, использующие чат

| Тип       | Объяснение                                                                      |
|-----------|---------------------------------------------------------------------------------|
| `LOWEST`  | Сообщение в чате будет обработано быстрее всех плагинов                         |
| `LOW`     | Сообщение в чате будет обработано после плагинов, у которых приоритет `LOWEST`  |
| `NORMAL`  | Сообщение в чате будет обработано после плагинов, у которых приоритет `LOW`     |
| `HIGH`    | Сообщение в чате будет обработано после плагинов, у которых приоритет `NORMAL`  |
| `HIGHEST` | Сообщение в чате будет обработано после плагинов, у которых приоритет `HIGH`    |
| `MONITOR` | Сообщение в чате будет только просмотрено                                       |

### `types`

Список всех чатов с их настройкой. Любой чат по умолчанию выглядит так и ты можешь добавлять свои

```yaml
название_чата:
  enable: true
  cancel: true
  range: PROXY
  priority: число
  trigger: "триггер"
  null_receiver:
    enable: true
    destination:
      type: "ACTION_BAR"
      times:
        stay: 20
  destination:
    type: "CHAT"
  cooldown:
    enable: false
  sound:
    enable: false
```

<Accordions>
    <Accordion title="Настройка">
        #### `enable`

        Включает работоспособность

        #### `cancel`

        - Если `true`, то ивент сообщения чата отменяется для других плагинов
        - Если необходимо, чтобы другой плагин, связанный с чатом (DiscordSRV например) - работал, то нужно ставить `false` (из-за этого появляется дублирование в консоль)
        ![chat console](/pulse/message_chat_console.webp)

        <include>../../../../parts/range.mdx</include>

        #### `priority`

        Приоритет чата, который используется для выбора, когда у нескольких чатов одинаковый `trigger`. Выбирается тот, у кого приоритет больше

        <Callout type="info" title="Информация">
            Например есть чаты:

            ```yaml
            admin:
              priority: 20
              trigger: "!"
            helper:
              priority: 10
              trigger: "!"
            ```

            Если игрок имеет право на оба чата, то плагином выбран будет `admin` т.к. у него больше `priority`
        </Callout>

        #### `trigger`

        Сообщение, с которого должно начинаться отправленное сообщение для чата

        <Callout type="info" title="Информация">
            Значение `trigger` может быть любым:
            - `!`, `admin`, `.f` и т.д.
            - пустым т.е. `trigger: ""`, тогда для этого чата подходит любое сообщение
        </Callout>

        В итоговом сообщении `trigger` удаляется, т.е. если игрок отправил `!привет`, в итоге будет `привет` без `!`

        #### `null_receiver`

        Включает сообщение "Тебя никто не услышал", если игрока никто не услышал

    </Accordion>
</Accordions>

<include>../../../../parts/destination.mdx</include>

<include>../../../../parts/cooldown.mdx</include>

<include>../../../../parts/sound.mdx</include>

<br></br>

# <include>../../../../parts/words.mdx#permission</include>
<span><include>../../../../parts/words.mdx#path</include> `permission.yml → message.chat`</span>

<include>../../../../files/minecraft/permission.yml#chat</include>

<include>../../../../parts/permission/permissionTier3.mdx</include>

### `types`

Список чатов и их прав

<include>../../../../parts/permission/cooldown.mdx</include>

<include>../../../../parts/permission/sound.mdx</include>