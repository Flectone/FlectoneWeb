...
<Callout type="info" title="Информация">
Например для сообщения о блокировке. Там есть плейсхолдер `<reason>`, значит я могу использовать `<reason>` внутри твич сообщений
</Callout>
...
Список, где ключом является имя канала, а значением список комманд, которые выполняться при старте/конце трансляции
...
- Комманд, при начале/конце трансляции, может быть сколько угодно и какие угодно
</Callout>
