---
title: Логгер
description: Настройка всего, что связано с консолью и логами
authors:
  - TheFaser
tag: "minecraft"
---
<Callout type="idea" title="Примечание">
    Если ты столкнулся с проблемой при включении плагина и лог запуска неполный, включи режим дебага с помощью флага `-Dflectonepulse.debug=true`, например `java -Dflectonepulse.debug=true -jar server.jar`
</Callout>
<Callout type="warn" title="Предупреждение">
    Здесь используются `ANSI` цвета для сообщений, обычные цвета не будут работать
</Callout>

<br></br>

# <include>../../../../parts/words.mdx#setting</include>
<span><include>../../../../parts/words.mdx#path</include> `config.yml → logger`</span>

<include>../../../../files/minecraft/config.yml#logger</include>

### `console`

Используется для отображения имени, когда сообщение создано консолью
![console](/pulse/config_console.webp)

### `prefix`

Префикс у всех сообщений, которые пишет `FlectonePulse` в консоль

### `description`

Сообщение при успешном старте, `<version>` заменится на текущую версию `FlectonePulse`

### `primary`

Цвет для выделения элементов в сообщении

### `warn`

Цвет для сообщений с предупреждением

### `info`

Цвет для сообщений с информацией

### `filter`

Идея взята [отсюда](https://github.com/Whitescan/ConsoleFilter/blob/master/src/main/java/dev/whitescan/consolefilter/share/LogFilter.java), спасибо @Whitescan

Списком указываются сообщения, которые нужно не показывать, используя ключевые слова, встречающиеся в сообщении

<Callout type="info" title="Информация">
Например я хочу отфильтровать

![filter](/pulse/config_logger_filter.webp)

Значит нужно вписать:
`Unknown or incomplete command` или `command` или `Unknown`
</Callout>
