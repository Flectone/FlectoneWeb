...
Settings for how long the task queue may hang before it is considered stalled

<Callout type="info" title="Information">
If there are tasks in the queue but none of them has finished during this time, the queue is cleared, the `FlectonePulse` threads are restarted and a warning is written to the console
</Callout>

#### `duration`

Time value for stall timeout

#### `time_unit`

Time unit for stall timeout