...
<Callout type="warn" title="Предупреждение">
   Если `<sprite:...>` напишет игрок в чате, то он всегда будет иметь белый цвет и его нельзя покрасить. Но если ты вписываешь `<sprite:...>` в конфигурации, он всегда будет окрашиваться в цвет, поэтому используй `<white>` перед `<sprite:...>`, чтобы цвет не применялся.
</Callout>
...
- Каждый атлас скачивается с собственного веб-сервера Flectone по ссылке `https://flectone.net/files/minecraft/r/minecraft/<version>/atlases/minecraft_textures_atlas_<atlas>.png.txt`
