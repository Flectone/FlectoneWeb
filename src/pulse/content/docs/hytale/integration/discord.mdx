---
title: Дискорд
description: Интеграция с Discord позволяет отправлять сообщения (Discord ↔ Hytale) между сервером Hytale и Discord сервером.
authors:
  - TheFaser
tag: "hytale"
---
![discord message](/pulse/integration_discord_message.webp)
![minecraft message](/pulse/integration_discord_message_minecraft.webp)

<Callout type="info" title="Информация">

Для корректной работы бота, включи три настройки для твоего бота в [Discord Developer Portal](https://discord.com/developers/applications/)

![develop](/pulse/integration_discord_developers.webp)

А также выдай права боту на `чтение сообщений`, `создание сообщений` и `использование webhooks`

</Callout>

<include>../../../../parts/messageTag.mdx</include>

<br></br>

# <include>../../../../parts/words.mdx#localization</include>
<span><include>../../../../parts/words.mdx#path</include> `localizations → язык.yml → integration.discord`</span>

<Tabs items={['ru_ru', 'en_us']}>
  <Tab value="ru_ru"><include>../../../../files/hytale/localizations/ru_ru.yml#discord</include></Tab>
  <Tab value="en_us"><include>../../../../files/hytale/localizations/en_us.yml#discord</include></Tab>
</Tabs>

### `null_player`

Сообщение, если введённый игрок через `custom_command` не найден

### `sender_name`

Название отправителя (самой интеграции), которое будет использовано, например, в другой интеграции для плейсхолдеров

### `format_reply`

Формат сообщения для тега `<reply>`, когда сообщение является ответом на другое

### `custom_command`

Список кастомных комманд интеграции, где ключом является название комманды, а значением её формат сообщения

<Callout type="info" title="Информация">
Пример

```yaml
custom_command:
  tps:
    content: "<tps>"
```

Комманда, чтобы получить значение `tps` на сервере. **Не забудь сделать её в inegration.yml**

</Callout>

#### `Плейсхолдеры`

Ты можешь использовать все плейсхолдеры, которые используются в начальном сообщении для Hytale
<Callout type="info" title="Информация">
    Например для сообщения о блокировке. Там есть плейсхолдер `<reason>`, значит я могу использовать `<reason>` внутри дискорд сообщений
</Callout>

Также есть плейсхолдеры, которые ТОЧНО будут заменяться в любом сообщении
- `<message>` сообщение, которое написал игрок
- `<final_message>` сообщение, отправленное в Hytale
- `<final_clear_message>` сообщение, отправленное в Hytale без unicode-смайлов
- `<player>` ник игрока, который отправил сообщение
- `<reply>` отформатированное сообщение-ответ, оно будет пустым, если основное сообщение не ответ
- Все плейсхолдеры из `PlaceholderAPI` и `FlectonePulse` тоже будут работать

У сообщения, которое будет отправлено из Дискорда в Hytale есть свои плейсхолдеры:

| Плейсхолдер      | Что возвращает                       |
|------------------|--------------------------------------|
| `<name>`         | Глобальное имя участника в Discord   |
| `<global_name>`  | Глобальное имя участника в Discord   |
| `<nickname>`     | Никнейм участника в Discord          |
| `<display_name>` | Отображаемое имя участника в Discord |
| `<user_name>`    | Тег участника в Discord              |
| `<role>`         | Наивысшая роль участника в Discord   |

### `info_channel`

Список айди каналов и их названий, для отображения какой-нибудь информации, например `TPS`

### `message_channel`

Список сообщений с их настройкой. Эти параметры ты должен добавлять вручную в файл локализации под каждым сообщением для дискорда, их можно пропускать и не использовать

```yaml
название_сообщения:
  content: ""
  webhook_name: "<player>"
  webhook_avatar: ""
  allowed_mentions:
    types:
      - ""
    users:
      - ""
    roles:
      - ""
  embed:
    color: ""
    title: ""
    url: ""
    author:
      name: ""
      url: ""
      icon_url: ""
    description: ""
    thumbnail: ""
    fields:
      - name: ""
        value: ""
        inline: false
    image: ""
    timestamp: true
    footer:
      text: ""
      icon_url: ""
```

<Callout type="info" title="Информация">

    Пример сообщения со всеми параметрами
    ![discord](/pulse/integration_discord_embed_preview.webp)

    Если параметр пустой или он не написан, то он не будет использован в итоговом сообщении

</Callout>

<Accordions>
    <Accordion title="Настройка сообщения">
        #### `content`

        Содержание сообщения
        ![discord content](/pulse/integration_discord_content.webp)

        #### `webhook_name`

        Название вебхука, можно использовать любой плейсхолдер. По умолчанию используется `<player>`

        #### `webhook_avatar`

        Включение дискорд вебхука с аватаром игрока
        ![discord webhook](/pulse/integration_discord_webhook.webp)

        #### `allowed_mentions`

        Кого из упомянутых в сообщении разрешено пинговать. Если параметр не написан, то не запингуется никто, даже если в сообщении есть `@everyone`

        ##### `types`

        Список типов упоминаний, которые разрешены

        | Тип                 | Что разрешает                       |
        |---------------------|-------------------------------------|
        | `USER`              | Упоминания пользователей            |
        | `ROLE`              | Упоминания ролей                    |
        | `EVERYONE_AND_HERE` | Упоминания `@everyone` и `@here`    |

        ##### `users`

        Список айди пользователей, которых разрешено пинговать

        ##### `roles`

        Список айди ролей, которые разрешено пинговать

        <Callout type="info" title="Информация">
            Пример, где пингуются только два человека из списка, а остальные упоминания остаются просто текстом

            ```yaml
            allowed_mentions:
              users:
                - "266531368914092042"
                - "735603638272000000"
            ```

            `types` и `users` (`roles`) работают вместе, `types` разрешает целый вид упоминаний, а `users` и `roles` — конкретных пользователей и роли
        </Callout>

        #### `embed`

        Дискорд эмбед сообщение
        ![discord embed](/pulse/integration_discord_embed.webp)

        ##### `color`

        Цвет `embed` сообщения

        ##### `title`

        Название `embed` сообщения

        ##### `url`

        Ссылка для `embed` сообщения

        ##### `author`

        <Accordions>
            <Accordion title="Автор `embed` сообщения">
                ###### `name`

                Название автора `embed` сообщения

                ###### `url`

                Ссылка на автора `embed` сообщения

                ###### `icon_url`

                Ссылка на аватарку автора `embed` сообщения. Если будешь менять на свою, то ссылка должна содержать `<skin>`
            </Accordion>
        </Accordions>
        <br/>
        ##### `description`

        Описание `embed` сообщения

        ##### `thumbnail`

        Маленькое изображение внутри `embed` сообщения

        ##### `fields`

        <Accordions>
            <Accordion title="Нижние поля внутри `embed` сообщения">
                ###### `name`

                Название поля внутри `embed` сообщения

                ###### `value`

                Значение поля внутри `embed` сообщения

                ###### `inline`

                Если включено, то поле будет расположен в столбик с другими полями
            </Accordion>
        </Accordions>
        <br/>
        ##### `image`

        Главное изображение `embed` сообщения

        ##### `timestamp`

        Если включено, то `embed` сообщение будет включать время создания

        ##### `footer`

        <Accordions>
            <Accordion title="Нижняя часть `embed` сообщения">
                ###### `text`

                Текст в нижней части `embed` сообщения

                ###### `icon_url`

                Изображение в нижней части `embed` сообщения
            </Accordion>
        </Accordions>
    </Accordion>
</Accordions>

<br></br>

# <include>../../../../parts/words.mdx#setting</include>
<span><include>../../../../parts/words.mdx#path</include> `integration.yml → discord`</span>

<include>../../../../files/hytale/integration.yml#discord</include>

<include>../../../../parts/enable.mdx</include>

<Callout type="warn" title="Предупреждение">
- Перед включением, вставь **токен** бота Discord
- После включения, **ЖЕЛАТЕЛЬНО** перезагрузить сервер, иначе плагин может вызвать зависание
</Callout>

### `ignore_all_bots`

Если включено, то любые сообщения пользователей, которые являются ботами, будут игнорироваться. Иначе будут игнорироваться только собственные сообщения от текущего бота

### `ignore_all_webhooks`

<Callout type="warn" title="Предупреждение">
    После отключения может возникнуть дублирование сообщений с другими плагинами, например с DiscordSRV
</Callout>

Если включено, то любые сообщения от Discord Webhook будут игнорироваться. Иначе будут игнорироваться только собственные Webhook сообщения

### `token`

[Токен](https://discordgsm.com/guide/how-to-get-a-discord-bot-token) дискорд бота для подключения. Можно использовать environment variables, например `${VALUE}`

### `proxy`

<include>../../../../parts/proxy.mdx</include>

### `custom_command`

Список кастомных комманд интеграции, ключом является название комманды и оно может быть любым. Комманды могут быть ТОЛЬКО информационными, на сервере они ничего не выполняют и не будут выполнять

| Параметр      | Объяснение                                                                      |
|---------------|--------------------------------------------------------------------------------|
| `need_player` | Проверять ли первый аргумент комманды на имя игрока, например `!ping TheFaser` |
| `aliases`     | Список псевдонимов для использования комманды                                  |

<Callout type="info" title="Информация">
Пример

```yaml
custom_command:
  tps:
    need_player: false
    aliases:
      - "!tps"
      - "!tickpersecond"
```

Комманда, чтобы получить значение `tps` на сервере с помощью `!tps` или `!tickpersecond`. **Не забудь сделать сообщение в локализации**

</Callout>

### `presence`

![discord presence](/pulse/integration_discord_presence.webp)

<Accordions>
    <Accordion title="Настройка статуса бота">
        #### `enable`

        Включает или выключает кастомный статус бота

        #### `status`

        | Режим            | Объяснение                       |
        |------------------|---------------------------------|
        | `UNKNOWN`        | -                               |
        | `ONLINE`         | В сети                          |
        | `DO_NOT_DISTURB` | В сети, с режимом не беспокоить |
        | `IDLE`           | В сети, но отошёл               |
        | `INVISIBLE`      | Невидимый                       |
        | `OFFLINE`        | Не в сети                       |

        #### `activity`

        Активность бота в дискорде

        ##### `enable`

        Включает или выключает активность

        ##### `type`

        | Тип         | Объяснение   |
        |-------------|-------------|
        | `UNKNOWN`   | -           |
        | `STREAMING` | Стримит     |
        | `LISTENING` | Слушает     |
        | `WATCHING`  | Смотрит     |
        | `CUSTOM`    | -           |
        | `COMPETING` | Соревнуется |

        ##### `name`

        Название активности

        ##### `url`

        Ссылка на активности
    </Accordion>
</Accordions>

### `channel_info`

![discord channel info](/pulse/integration_discord_channel_info.webp)

<Accordions>
    <Accordion title="Настройка каналов с информацией">
        #### `enable`

        Нужен ли канал информации

        <include>../../../../parts/ticker.mdx</include>
    </Accordion>
</Accordions>

### `message_channel`

Список типов сообщений и [ID каналов](https://support.discord.com/hc/ru/articles/206346498-%D0%93%D0%B4%D0%B5-%D0%BC%D0%BD%D0%B5-%D0%BD%D0%B0%D0%B9%D1%82%D0%B8-ID-%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D1%82%D0%B5%D0%BB%D1%8F-%D1%81%D0%B5%D1%80%D0%B2%D0%B5%D1%80%D0%B0-%D1%81%D0%BE%D0%BE%D0%B1%D1%89%D0%B5%D0%BD%D0%B8%D1%8F) в Discord

<Callout type="info" title="Информация">
Например я хочу, чтобы из Hytale отправлялось сообщение комманды `/ban` в Discord
<Steps>
    <Step>
        Копирую ID канала в дискорде `1286666844358316083`
    </Step>
    <Step>
        Прописываю `COMMAND_BAN: "1286666844358316083"`
    </Step>
</Steps>

```yaml
message_channel:
  COMMAND_BAN: 
    - "1286666844358316083"
```
</Callout>

Локализацию можно не настраивать, по умолчанию сообщение будет отправляться с форматом `<final_message>`

<include>../../../../parts/hytale/destination.mdx</include>

<include>../../../../parts/hytale/sound.mdx</include>

<br></br>

# <include>../../../../parts/words.mdx#permission</include>
<span><include>../../../../parts/words.mdx#path</include> `permission.yml → integration.discord`</span>

<include>../../../../files/hytale/permission.yml#discord</include>

<include>../../../../parts/permission/permissionTier3.mdx</include>

<include>../../../../parts/permission/sound.mdx</include>