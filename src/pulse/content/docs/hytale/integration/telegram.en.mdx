---
title: Telegram
description: Integration with Telegram allows for sending messages (Telegram ↔ Hytale) between the Hytale server and Telegram.
authors:
  - TheFaser
tag: "hytale"
---
![telegram message 1](/pulse/integration_telegram_message_1.webp)
![telegram message 2](/pulse/integration_telegram_message_2.webp)
![minecraft message](/pulse/integration_telegram_message_minecraft.webp)
...
<Tabs items={['en_us', 'ru_ru']}>
    <Tab value="en_us"><include>../../../../files/hytale/localizations/en_us.yml#telegram</include></Tab>
    <Tab value="ru_ru"><include>../../../../files/hytale/localizations/ru_ru.yml#telegram</include></Tab>
</Tabs>
...
<Callout type="info" title="Information">
    Example for a ban message. There is a `<reason>` placeholder there, which means I can use `<reason>` inside Telegram messages.
</Callout>
...
A message sent from Telegram to Hytale has its own placeholders:
...
| `<role>`       | Telegram user's status (role)                             |
...
<include>../../../../files/hytale/integration.yml#telegram</include>
...
If enabled, messages will arrive in Telegram silently, without a notification

### `protect_content`

If enabled, messages cannot be forwarded or copied
...
<Callout type="info" title="Information">
For example, I want Hytale `/ban` command messages to be sent to Telegram
...
There can be any number of chats, as long as the bot from [`token`](/pulse/docs/hytale/integration/telegram/#token) has access to them. Localization configuration is optional; by default, the message will be sent with the `<final_message>` format.
</Callout>
...
<include>../../../../files/hytale/permission.yml#telegram</include>
