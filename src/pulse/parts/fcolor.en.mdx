<Callout type="info" title="How does `<fcolor:...>` work?">
    `<fcolor:...>` is a placeholder that is replaced by values from [`default_colors`](#default_colors) or, if the player has their own, by the player's custom colors.\
    \
    The value of `<fcolor:...>` can be anything, but the command only allows writing colors of any format and `<gradient:...:...>`. There can be any number of these values; the maximum count is determined by the highest digit in [`default_colors`](#default_colors).\
    \
    ...
    \
    ![fcolor](/pulse/message_format_fcolor_info.webp)
</Callout>