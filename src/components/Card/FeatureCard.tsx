...
export default function FeatureCard({ className, image, title, description, link, glow, imagePosition, imageWidth, imageHeight, imageAlt }: FeatureCardProps) {
    return (
        <Card path={link} className={`${className} not-dark:border`}>
            <ThemeImage src={image}
                   width={imageWidth}
                   height={imageHeight}
                   className={`${imageStyles[imagePosition]} text-fd-primary rounded-lg absolute -z-1 `}
