...
export async function imageToHeadSkin(file: File) {
  ...
  const [imageHeignt, imageWidth] = [(await sharp(image).metadata()).height, (await sharp(image).metadata()).width]
  ...
  for (let [y, posY] = [0, 0]; y < imageHeignt; [y, posY] = [y+8, posY+1]) {
    for (let [x, posX] = [0, 0]; x < imageWidth; [x, posX] = [x+8, posX+1]) {
      const width = 8
      const height = 8
  ...
  if (processedData?.imageArray) {
    for (const item of processedData.imageArray) {
      try {
        ...
        myHeaders.append("Authorization", `Bearer ${process.env.MINESKIN_API_TOKEN}`);

        const fetchResponse = await fetch("https://api.mineskin.org/v2/generate", {
          method: "POST",
          headers: myHeaders,
          body: JSON.stringify({
            variant: "classic",
        ...
        console.error(`Error creating skin for block ${item.x}:${item.y}`, error);
      }
    }
  }
